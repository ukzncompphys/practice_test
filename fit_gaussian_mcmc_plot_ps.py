...
if __name__=='__main__':
    ...
    #why do I cut out the first element,and the second half of the FFT?
    chainft=numpy.fft.fft(chain[:,0])
    chainft=chainft[1:chainft.size/2]
    ps=numpy.abs(chainft)**2
    #why do I have an x vector here that doesn't start at zero?
    #hint - I'll be making the plot have log-log axes
    x=numpy.arange(1,ps.size+1)
    plt.plot(x,ps)
    ax=plt.gca()
    ax.set_xscale('log')
    ax.set_yscale('log')
